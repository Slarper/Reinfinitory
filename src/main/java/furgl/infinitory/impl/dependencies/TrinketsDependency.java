	...
	public void adjustTrinketSlots(ScreenHandler handler, int difference, InfinitorySlot slot) {
		try {
			...
			if (handler instanceof TrinketPlayerScreenHandler) {
				...
				Map<EntityType<?>, Map<String, SlotGroup>> slots = (Map<EntityType<?>, Map<String, SlotGroup>>) slotsField.get(EntitySlotLoader.CLIENT);
				...
				((TrinketPlayerScreenHandler) handler).trinkets$updateTrinketSlots(true);
			}
		}
		catch (Exception e) {
			e.printStackTrace();
